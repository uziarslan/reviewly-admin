...
export default function WhatsNew() {
  ...
  const [existingImageUrl, setExistingImageUrl] = useState(''); // current saved image
  ...
  const load = async () => {
    ...
    setError(null);
    try {
      const res = await whatsNewAPI.getAll();
  ...
  const submit = async (status) => {
    ...
    try {
      ...
      const res = editingId
        ? await whatsNewAPI.update(editingId, fd)
        : await whatsNewAPI.create(fd);
      if (res.success) {
        setSavedMsg(
  ...
  const togglePublish = async (entry) => {
    try {
      ...
      const res = await whatsNewAPI.update(entry._id, fd);
      if (res.success) load();
  ...
  const showImage =
    previewUrl || (!removeImage && existingImageUrl ? existingImageUrl : '');
    ...
    <div className="min-h-screen bg-[#F5F4FF]">
      ...
      <main className="max-w-[1440px] mx-auto px-4 sm:px-6 lg:px-20 py-6">
        ...
        <p className="font-inter text-[13px] text-[#6C737F] mb-6">
          Post product updates that appear on the user dashboard. Publishing an
          entry triggers the yellow "new update" dot for users.
        </p>

        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          {/* ── Form ───────────────────────────── */}
          <div className="bg-white rounded-[12px] border border-[#E5E7EB] p-6 h-fit">
            <div className="flex items-center justify-between mb-5">
              ...
              {editingId && (
                <button
                  type="button"
                  onClick={resetForm}
                  className="font-inter text-[13px] text-[#6E43B9] hover:underline"
                >
                  + Create new instead
                </button>
              )}
            ...
            <div className="space-y-4">
              ...
              <label className="flex items-center gap-2.5 cursor-pointer select-none">
                <input
                  ...
                  onChange={(e) =>
                    setForm((f) => ({ ...f, feedbackTag: e.target.checked }))
                  }
                  className="w-4 h-4 accent-[#6E43B9]"
                />
                <span className="font-inter text-[13px] text-[#0F172A]">
                  Show the "BASED ON USER FEEDBACK" pill
                </span>
              </label>
                ...
                {showImage && (
                  <div className="relative mb-3 inline-block">
                    <img
                      src={showImage}
                      alt="preview"
                      className="max-h-48 rounded-[8px] border border-[#E5E7EB]"
                    />
                    <button
                      type="button"
                      onClick={() => {
              ...
              {savedMsg && (
                <p className="font-inter text-[13px] text-[#16A34A]">
                  {savedMsg}
                </p>
              )}
          ...
          {/* ── List ───────────────────────────── */}
          <div>
            <h2 className="font-inter font-bold text-[16px] text-[#0F172A] mb-4">
              All entries{' '}
            ...
            {loading ? (
              ...
              <p className="font-inter text-[14px] text-[#6C737F]">
                No entries yet.
              </p>
            ) : (
                ...
                {entries.map((e) => (
                  ...
                  >
                    <div className="flex items-center gap-2 mb-2 flex-wrap">
                      <span
                        className={`font-inter font-semibold text-[11px] px-2.5 py-1 rounded-full ${TYPE_PILL[e.type]}`}
                      >
                      ...
                      <span className="font-inter text-[12px] text-[#9CA3AF] ml-auto">
                        {e.status === 'published'
                          ? prettyDate(e.publishedAt)
                          : `Created ${prettyDate(e.createdAt)}`}
                      </span>
                    </div>
                    <p className="font-inter font-bold text-[15px] text-[#0F172A]">
                      {e.title}
                    </p>
                    <p className="font-inter text-[13px] text-[#6C737F] whitespace-pre-line line-clamp-3 mt-0.5">
                      {e.description}
                    </p>
                    {e.imageUrl && (
                      <img
                        src={e.imageUrl}
                        alt=""
                        className="mt-2 max-h-32 rounded-[8px] border border-[#F2F4F7]"
                      />
                    )}
                    <div className="flex gap-2 mt-3 pt-3 border-t border-[#F2F4F7]">
                      <button
                        type="button"
                        onClick={() => startEdit(e)}
                        className="font-inter font-medium text-[12px] text-[#6E43B9] hover:underline"
                      >
                        Edit
                      </button>
                      <button
                        ...
                        className="font-inter font-medium text-[12px] text-[#0F172A] hover:underline"
                      >
